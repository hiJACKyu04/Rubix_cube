		...
		for (let i = 0; i < steps.length; i++) {
			...
			if(step != "") {
				...
				if (i != steps.length - 1) {
					...
					typewriter.typeString(stepName[i] + ": " + step + " | Total: " + totalMoves + " moves").start();
				}
			}
			else {
				if (i != steps.length - 1) {
			...
			console.log(step);
			if(step != "") {
				console.log(stepName[i] + " completed.");
			} else {
				console.log(stepName[i] + " was already solved.");
			}
		}
	}
